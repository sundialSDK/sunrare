...
class ArtworkNode: SCNNode {
    ...
    init(model: ArtworkModel, contentInfo: [ArtworkSubNodeType: Any]) {
        ...
        //set correct size
        let max = max(model.imageSize.width, model.imageSize.height)
        let realSize = CGSize(width: model.imageSize.width/max, height: model.imageSize.height/max)
        ...
        //load image
        loadImage(url: URL(string: model.imageLink))
